...
export type TranscriptBlock =
	...
	| { readonly kind: 'thinking'; readonly text: string; readonly title: string }
	| { readonly kind: 'activity'; readonly activity: TranscriptActivity };
...
function normalizeTurn(request: JsonObject, index: number): TranscriptTurn {
	...
	for (let partIndex = 0; partIndex < response.length; partIndex++) {
		...
		if (!kind && markdownValue(part.value)) {
			...
			textRun.push(markdownValue(part.value)!);
			continue;
		}
		...
		if (kind === 'thinking') {
			...
			if (value) {
				flushTextRun();
				blocks.push({ kind: 'thinking', text: value, title: title ?? '' });
			}
			continue;
		}
		...
		if (kind === 'progressMessage') {
			...
			if (value) {
				const progress: TranscriptActivity = {
				...
				flushTextRun();
				blocks.push({ kind: 'activity', activity: progress });
			}
			continue;
		}
		...
		if (kind === 'toolInvocationSerialized') {
			...
			if (terminalData) {
				...
				const terminalActivity: TranscriptActivity = {
					...activity,
				...
				activities.push(terminalActivity);
				flushTextRun();
				blocks.push({ kind: 'activity', activity: terminalActivity });
			} else {
	...
	return {
		id: requestId ?? `request-${index}`,
		editable: requestId !== undefined,
